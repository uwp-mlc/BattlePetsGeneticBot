...
import control.TextRoundControl;
...
public class DamageCalculator implements Calculatable
{
	...
	public Damage calculateDamage(int attackingPlayerIndex, int victimPlayerIndex)
	{
		...
		// THRILL OF VICTORY HACK
		
		double extraRandomDamage = roundControl.thrillOfVictoryHack(attackingPlayerIndex);
		double randomDamage = damage.getRandomDamage();
		damage.setRandomDamage(randomDamage+extraRandomDamage);
	...
	{
		return roundControl.isSkillRecharing(playerIndex, skill);
	}
